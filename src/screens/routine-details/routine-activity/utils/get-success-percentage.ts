...
import { getDay, getWeek, isSameDay, isSunday, startOfWeek } from 'date-fns'
...
// DAY
export const getDaySuccessPercentage = ({ days, recurrence, actions, occurrence }: Props) => {
...
// WEEK
export const getWeekSuccessPercentage = ({ days, recurrence, actions, occurrence }: Props) => {
  if (!recurrence.length) return 100
  ...
  days.forEach((day) => {
    ...
    if (recurrence.includes(getWeek(day) % 2)) scheduledWeeks = scheduledWeeks + 1
...
export const getSuccessPercentage = ({
  ...
  if (scheduleType === SCHEDULE_TYPES.daily) return getDaySuccessPercentage({ actions, occurrence, days, recurrence })
  if (scheduleType === SCHEDULE_TYPES.weekly) return getWeekSuccessPercentage({ actions, occurrence, days, recurrence })
  return 0
}
